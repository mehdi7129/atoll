> **ARCHIVE HISTORIQUE — arrêtée avant la release v0.18.0.**
> Les états, versions et tâches ci-dessous sont datés ; ils ne décrivent pas
> la situation courante. Reprendre avec [HANDOFF.md](HANDOFF.md) et `CLAUDE.md`.
...
| Build installé | `~/Applications/Atoll.app` = **Release NOTARISÉE v0.16.6** (bundle 31), **installée par Sparkle** — première mise à jour de bout en bout constatée sur cette machine — et contrôlée le 2026-09-04 : `spctl : accepted — Notarized Developer ID`, staple validé, `codesign --verify --deep --strict` sans résidu, app lancée, helper `~/.atoll/bin/atoll-bridge` répondant (`recall-stats`, exit 0) et journal du recall intact (740 passages du 2026-08-10 au 2026-09-04). La chaîne de hook n'a PAS été rejouée cette fois : un `UserPromptSubmit` de test écrirait une ligne dans le journal et fausserait le dénominateur du 9 septembre. Sparkle remplace EN PLACE : la 0.16.5 n'a pas de copie conservée, contrairement aux installations manuelles — les bundles `~/Applications/Atoll-<version>-remplacee-*.app` (0.16.0, 0.16.2, 0.16.4) restent à mettre à la corbeille quand la 0.16.6 aura fait ses preuves. Pas de build Debug installé — pour reprendre la boucle de dev, l'installer en `~/Applications/Atoll-dev.app`, jamais `ditto` par-dessus la Release (`ditto` FUSIONNE, un `Atoll.debug.dylib` résiduel casse le sceau) |
