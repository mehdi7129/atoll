...
public final class MemoryIndex {
    ...
    private static func searchSQL(roles: Set<TranscriptLine.Role>?) -> String {
        ...
        WHERE messages_fts MATCH ?1
          -- `m.role = 'note'` : les NOTES d'apprentissage d'Atoll sont indexées
          -- sans cwd (elles ne viennent d'aucun projet — elles valent pour
          -- TOUS). Sans cette clause, la comparaison rendait NULL et le filtre
          -- de projet les excluait toutes : le réglage « limiter au projet
          -- courant » étant ON par défaut, la mémoire longue durée que produit
          -- Atoll n'était JAMAIS injectée (audit du 2026-07-27).
          --
          -- Le discriminant est le RÔLE, pas « project_path IS NULL » : mesuré
          -- sur la vraie base, 20 sessions ont un project_path nul dont 3 sont
          -- de VRAIES sessions (fichiers trop courts pour porter un cwd). Les
          -- laisser passer rouvrait la fuite inter-projets que la frontière de
          -- chemin ferme (revue des corrections).
          AND (?2 IS NULL OR m.role = 'note'
